...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [NetWorking goodsDetailWithApi:str success:^(GoodsDetail *model) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.dianCha.text = model.symbol; ;
            self.xioaSHUwei.text = model.digits;
            self.zhisunSHuiping.text = model.profit_mode;
            self.guadanTOquxiao.text = model.stops_level;
            self.heyueSHU.text = model.contract_size;
            self.lirunJisuanMOshi.text = model.tick_size;
            self.kucunFeileixing.text = model.tick_value;
            self.mairuKUcunfei.text = model.margin_mode;
            self.maichuKucunfei.text = model.margin_initial;
            //self.yufukuanJisuan;
